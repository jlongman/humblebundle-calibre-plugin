...
class HBDDialog(QDialog):
    ...
    def Import(self):
        ...
        skip_count = 0
        try:
            skip_count = int(prefs['skip_count'])
        except:
            self.textlog.append('Skip Count meaningless, using 0')
        self.textlog.append('Skipping %s keys' % skip_count)
        ...
        for key in key_downloads:
            ...
            getkeys = key_downloads.get(key)
            if len(getkeys) >= skip_count:
                HumbleDownload.update_download_list_url(hapi, getkeys)
                ...
                ticker += len(getkeys)
                skip_count -= len(getkeys)
                self.textlog.append(
                    '(%s/%s) Skipping key %s with %s entries... skipcount %s' % (ticker, num_new_books, key, len(getkeys), skip_count))
